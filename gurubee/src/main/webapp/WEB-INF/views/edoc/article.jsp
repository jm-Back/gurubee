				...
				<table class="table table-border table-form">
					...
					<tr>
						...
						<tr>
							...
							<td>
							<c:forEach var="empdto" items="${empdto}" varStatus="status">
								
									<div>${empdto.name_apper}&nbsp;${empdto.pos_name}</div> 
									<c:if test="${empdto.app_result == 1}">
										<button class="btn btn-success" disabled
											style="border-radius: 20px;">승인</button>
									</c:if> <c:if test="${empdto.app_result == 0}">
										<button class="btn btn-success" disabled
											style="border-radius: 20px;">대기</button>
									</c:if> <c:if test="${empdto.app_result == -1}">
										<button class="btn btn-warning" disabled
											style="border-radius: 20px;">반려</button>
									</c:if> <input type="hidden" id="id_apper${status.count}"
									name="id_apper" value="${empdto.id_apper}">
     						 ...
     						 <div class="modal-body">
        							...
        							<div> 
        								<div class="form-check">
  											<input class="form-check-input" type="radio" name="resultRadio" id="resultRadio_Y" value="1" checked>
  												<label class="form-check-label" for="flexRadioDefault1" id="result_1">
   													승인
  												</label>
										</div>
										<div class="form-check">
  											<input class="form-check-input" type="radio" name="resultRadio" id="resultRadio_N" value="-1">
  												<label class="form-check-label" for="flexRadioDefault2" id="result_2">
   													반려
 												</label>
										</div>
