				...
				<div class="body-main">
					...
					<form name="listForm" method="post" enctype="multipart/form-data">
							...
							<tbody>
								...
								<c:forEach var="dto" items="${list}" varStatus="status">
									<tr>
										<td>${dto.app_num}</td>
										<td>${dto.app_doc}</td>
										<td><c:if test="${fn:contains(dto.result, '반려')}">
												<button class="btn btn-warning" disabled
													style="border-radius: 20px;">${dto.result}</button>
											</c:if> <c:if test="${fn:contains(dto.result, '승인')}">
												<button class="btn btn-success" disabled
													style="border-radius: 20px;">${dto.result}</button>
											</c:if> <c:if test="${fn:contains(dto.result, '대기')}">
												<button class="btn btn-success" disabled
													style="border-radius: 20px;">${dto.result}</button>
											</c:if></td>
										<td>${dto.result_name}</td>
										<td><c:if test="${dto.temp == -1}">
												<button class="btn btn-outline-success" disabled
													style="border-radius: 20px; width: 85px;">수정 이력</button>
											</c:if> <a href="${articleUrl}&app_num=${dto.app_num}">${dto.title}</a>
										</td>
										<td>${dto.app_date}</td>
										<td><c:if test="${fn:contains(dto.result, '반려')}">
												<button class="btn btn-warning" disabled
												...
												<button class="btn btn-success" disabled
													style="border-radius: 20px;">진행중</button>
											</c:if></td>
									</tr>
						...
						<div style="float: right;">
							...
							</a> <a href="${pageContext.request.contextPath}/edoc/list_temp.do">
								<button type="button" id="btnTempEdoc" class="btn btn-success"
									style="height: 35px;">임시보관함</button>
							</a>
						</div>
